...
- **游戏内合成预览（只读）**：把当前动画和另一个动画按游戏的方式拼成上下半身（以粒子 8 对齐），并可拖动上身扭转角（游戏里奔跑最多约 37°、走路约 60°，瞄准时不限，最大 180°）。预览期间不能拖粒子，编辑数据不受影响。上身层是可选的：摔倒、飞扑、爬梯、游泳、趴下起身、投降、跳伞、受伤这些动作游戏里是整段播的，选到它们时会提示"合成只是假设"。
