...
- 角色朝向：游戏先把角色转到面朝方向再算骨段朝向，骨段坐标轴不正交时同一个动作换个朝向蒙皮就不一样（vanilla 士兵几乎看不出，轴歪的异形骨可能差几十体素）。动画面板「蒙皮规则」下方的「角色朝向」滑杆可以按任意朝向预览，模型原地不动、只显示差异；「回正」回到 0。
...
- **结构检查**：粒子不足 13 个、体素绑在下标 ≥ 17 的骨段、上臂骨段端点与游戏取朝向的粒子对不一致、没有上半身层粒子、**bind 姿态下就会错位的骨段**（参考方向与骨段几乎平行时，游戏里这段体素静止时就已错位；按相对误差判定，长骨段偏几个体素不会误报）、**游戏里左右两侧不会互为镜像的骨段对**等。绑骨工具面板也会显示。
- **左右对称优化**（绑骨工具）：游戏按骨段下标选蒙皮规则，只有部分下标对的规则左右互为镜像。「左右对称优化...」可载入模型自己的动画，按多个角色朝向评估（游戏在转到面朝方向之后才算骨段朝向，坐标轴不正交时结果随朝向变化），搜出一套左右差异和跟随误差都尽量小的槽位分配，列出每对骨段现状与建议的代价，一键应用（可撤销）。骨段面板里也能手动「对调方向」「交换下标」，体素绑定跟着骨段走。
- **逐骨段规则体检**：每根骨段套用的游戏规则、引用的粒子、静止错位（单独一列）、本帧是否退化（参考方向与骨段平行时游戏里体素会塌缩或翻转）与畸变；「扫描整段动画」找出最差的一刻，点击跳过去；可在视口把需注意 / 异常的骨段标成橙 / 品红。
- **上下半身分层**：游戏用 `bodyAreaHint` 决定粒子属于哪一层——`2` 上半身（跟瞄准方向转、可被换弹等上身动画替换），其它为下半身（跟移动方向转）。可按层给粒子着色；绑骨工具的粒子属性里也改成了下拉框。
- **游戏内合成预览（只读）**：把当前动画和另一个动画按游戏的方式拼成上下半身（以粒子 8 对齐），并可拖动上身扭转角（游戏里奔跑最多约 37°、走路约 60°；瞄准移动时约 93°，再大就改为倒退走、腿转到准星一侧）。预览期间不能拖粒子，编辑数据不受影响。上身层是可选的：摔倒、飞扑、爬梯、游泳、趴下起身、投降、跳伞、受伤这些动作游戏里是整段播的，选到它们时会提示"合成只是假设"。

### 游戏外观

「视图...」里的「游戏外观（点精灵）」按游戏的方式画体素：屏幕对齐的方块带黑色描边，颜色调亮（饱和度 ×1.05、亮度 ×1.28），方块下半部压暗。不含场景光照与雾。

精灵大小默认按**像素**算，和游戏一致——游戏把边长按档位写死（高细节描边 4.2 / 本体 2.6，低细节 3.6 / 2.4）且关掉了距离衰减，所以镜头拉近体素不会跟着变大。也因此只有在游戏的镜头距离、且窗口高度与游戏相近时才和实机一致：旁边的「游戏镜头」按钮一键设成透视 + 距离 1152 体素 + 俯角 58.4°。想要随缩放变化的旧行为，把「大小按像素」关掉即可。
