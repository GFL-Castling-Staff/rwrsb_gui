...
- **bind 往返误差**：bind 姿态下若某根骨段的参考方向与骨段几乎平行，bind 四元数明显非单位，`Inverse` 与正向变换不再抵消，游戏里这段体素静止时就已错位（例如某自定义骨架的小腿错位 27 体素）。`EngineSkinBinding.bind_error` / `bind_drift` 记录它，体检按它打分，结构检查（`bind_warnings`）列出这些骨段。退出动画模式时体素直接换回 canonical，而不是"在 bind 姿态再蒙皮一次"。
