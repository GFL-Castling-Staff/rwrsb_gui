...
**角色朝向**：游戏先按角色朝向把粒子写进世界坐标，再用世界坐标算身体参考系和骨段朝向；bind 却用模型 XML 里未旋转的骨架。坐标轴不正交时 `FromRotationMatrix` 不随旋转协变，所以同一个动作面朝不同方向，蒙皮结果不同：vanilla 士兵中位 0.05 体素、P95 1.2、最大约 14 体素（前臂）；轴不正交的异形骨可达几十体素。上面说的"严格镜像"只在默认朝向（0° / 180°）成立。动画工具的预览按默认朝向算；`SymmetryEvaluator` 按 `SYM_HEADINGS`（0 / 45 / 90 / 135 度）把每个姿态转到世界坐标再评估，镜像面随朝向一起转。
