...
- **Bind round-trip error**: if a stick's reference direction is nearly parallel to the stick in the bind pose, the bind quaternion is clearly non-unit and `Inverse` no longer cancels the forward transform, so in game that stick's voxels are displaced even at rest (e.g. 27 voxels on one custom skeleton's shin). `EngineSkinBinding.bind_error` / `bind_drift` record it, the inspector grades it, and the structure checks (`bind_warnings`) list those sticks. Exiting animation mode restores the canonical voxels directly instead of "skinning once more at the bind pose".
