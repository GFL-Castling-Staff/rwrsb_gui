...
Because the game picks rules by index, a left/right pair of sticks does not necessarily come out as mirror images. Mirroring a pose exactly (x negated, left and right particles swapped) and comparing voxel by voxel: the leg rules (0|1 with 3|4), upper arms (12, 10) and chest (6, 9) are exact mirrors (when both sides point the same way); forearms (13, 11) are while the chest keeps its shape; (5, 8) and (14, 15) use the same rule on both sides, so whenever the axes fed to `FromAxes` are not orthogonal the two sides differ, and reversing a stick does not help.
...
- Structure check `asymmetric_pairs`: `EditorState.symmetry_structure_warnings()` estimates it with synthetic poses (`synthetic_poses`: chest particles 1, 2, 3, 8 turned rigidly, the rest perturbed; a deforming chest would wrongly flag the forearm slots), cached per skeleton and bindings.
