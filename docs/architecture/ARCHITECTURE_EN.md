...
- **Quaternion math follows OGRE 1.7**: `FromAxes` does not orthonormalize its input, so the quaternion may be non-unit and voxels deform slightly non-rigidly. The game really does this, so it is reproduced. The engine rule itself costs about 0.8 ms per frame; the whole update including writing voxels back is about 2.5 ms (vanilla model; legacy about 2.2 ms).
- **Bind round-trip error**: if a stick's reference direction is nearly parallel to the stick in the bind pose, the bind quaternion is clearly non-unit and `Inverse` no longer cancels the forward transform, so in game that stick's voxels are displaced even at rest (e.g. 27 voxels on one custom skeleton's shin). `EngineSkinBinding.bind_error` / `bind_drift` record it. It does not depend on the pose and is graded on its own (`bind_grade`): by the relative error (roughly drift / distance of the voxels from the a end), and only once the drift is big enough to see - over 5% and at least 0.5 voxel is Caution, over 15% and at least 2 voxels is Bad; 4 voxels on a 150-voxel leg (3%) is not reported, 4 voxels on a 20-voxel arm is. The rule table shows it in its own "At rest" column, while "Now" and "Worst" are coloured by the pose alone (`frame_grade`); the structure checks (`bind_warnings`) list those sticks. In the binding tool the inspector's bind now includes the voxels so the drift can be measured (binding edits bump `_bindings_rev`, which the caches key on). Exiting animation mode restores the canonical voxels directly instead of "skinning once more at the bind pose".
...
#### Left/right symmetry and slot assignment

Because the game picks rules by index, a left/right pair of sticks does not necessarily come out as mirror images. At the default facing, mirroring a pose exactly (x negated, left and right particles swapped) and comparing voxel by voxel: the leg rules (0|1 with 3|4), upper arms (12, 10) and chest (6, 9) are exact mirrors (when both sides point the same way); forearms (13, 11) are while the chest keeps its shape; (5, 8) and (14, 15) use the same rule on both sides, so whenever the axes fed to `FromAxes` are not orthogonal the two sides differ, and reversing a stick does not help.

**Facing**: the game writes the particles in world space already turned to the soldier's facing, then builds the body frame and stick orientations from those world positions, while the bind uses the unrotated skeleton from the model XML. With non-orthogonal axes `FromRotationMatrix` does not commute with rotation, so the same animation skins differently depending on which way the soldier faces: vanilla median 0.05 voxel, 95th percentile 1.2, worst about 14 (forearms); custom skeletons with skewed axes reach tens of voxels. The "exact mirror" above therefore only holds at the default facing (0 / 180 deg). The animation tool's preview uses facing 0 by default; the "Facing" slider in the animation panel (`EditorState.preview_heading_deg` / `set_preview_heading`) changes it: engine skinning turns the displayed pose to that facing in world space, skins it, and turns the result back (the model stays put, only the facing-dependent differences show); the per-frame inspector and the whole-animation scan use it too; "Reset" goes back to 0. `SymmetryEvaluator` turns each pose to the facings in `SYM_HEADINGS` (0 / 45 / 90 / 135 deg) in world space before scoring, with the mirror plane turned along.

Symmetry alone is not enough, though: the same pair in different slots differs a lot in how closely the rendered voxels follow the stick itself. `engine_skin.SymmetryEvaluator` scores "stick x slot x origin end" as visible error in voxels: left/right difference (counted double), following error and distortion, maxed per pose and taken at the 95th percentile, then maxed with the rest drift. A stick's orientation depends only on its slot rule, its endpoints and the body frame, not on how the other sticks are assigned, so `optimise_slots` solves it as an assignment problem by exhaustive search: mirror pairs into slot pairs (which side goes into the right slot, which end is the origin), centre / unpaired sticks into the remaining slots. New placements with more than 5 voxels of rest drift are ruled out, and keeping a placement counts 2 voxels less so the search does not reshuffle for a tiny gain.

- Structure check `asymmetric_pairs`: reported when the difference exceeds 10% of the stick's voxel reach and at least 2 voxels. `EditorState.symmetry_structure_warnings()` estimates it with synthetic poses (`synthetic_poses`: chest particles 1, 2, 3, 8 turned rigidly, the rest perturbed; a deforming chest would wrongly flag the forearm slots), cached per skeleton and bindings.
- Binding tool "Symmetry optimizer...": `symmetry_inputs()` snapshots the inputs on the main thread, a background thread loads the animations (name filter; particle count and size filtered automatically) and searches while the UI keeps running; the result carries `slot_signature()` and cannot be applied once the skeleton or bindings have changed. `apply_slot_plan()` reorders sticks and bindings in one undoable step.
- Stick panel "Reverse direction" / "Swap index": `reverse_stick()` / `swap_sticks()`; bindings move with the sticks.

### bodyAreaHint decides the upper/lower animation layer

In game, particles with `bodyAreaHint == 2` belong to the upper-body layer: they turn with the aim direction, and while an upper-body animation plays (aiming, reloading...) their positions come from that animation, aligned at particle 8. Any other value is the lower-body layer, which turns with the movement direction. Running lets the upper body twist up to about 37° from the legs, walking about 60°; when moving while aiming, past about 93° the game switches to walking backwards and turns the legs to face against the movement direction (toward the crosshair), so the steady twist stays within about 93° and only briefly approaches 180° while turning. The binding tool edits the hint with a dropdown, and both tools can "Color by body layer".
