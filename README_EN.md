...
- Facing: the game turns the soldier to its facing before it works out stick orientations, so when a stick slot's axes are not orthogonal the same animation skins differently at different facings (barely visible on vanilla soldiers, tens of voxels on some custom skeletons). The "Facing" slider under "Skinning rule" in the animation panel previews any facing with the model kept in place, showing only the differences; "Reset" goes back to 0.
...
- **Structure checks**: fewer than 13 particles, voxels bound to stick index ≥ 17, upper-arm sticks whose endpoints differ from the particle pair the game takes their orientation from, no upper-body-layer particles, **sticks already displaced at the bind pose** (when a reference direction is nearly parallel to the stick, the game shows those voxels out of place even at rest; graded by relative error, so a long stick a few voxels off is not flagged), **left/right stick pairs that will not mirror each other in game**, and so on. Also shown in the binding tool's panel.
- **Symmetry optimizer** (binding tool): the game picks each stick's skinning rule by index, and only some index pairs are mirror images of each other. "Symmetry optimizer..." can load the model's own animations, evaluate several facings (the game computes stick orientations after turning the soldier to its facing, and with non-orthogonal axes the result changes with it), search for a slot assignment that keeps left/right differences and following error low, list each pair's current and suggested cost, and apply it in one undoable step. The stick panel can also "Reverse direction" and "Swap index" by hand; voxel bindings move with the sticks.
- **Per-stick rule inspector**: the game rule each stick gets, the particles it references, its offset at rest (own column), and whether the current frame is degenerate (a reference parallel to the stick makes the voxels collapse or flip in game) or distorted; "Scan whole animation" finds the worst moment and jumps there on click; flagged sticks can be highlighted orange / magenta in the viewport.
...
- **In-game composite preview (read-only)**: combines the current animation with another one into upper and lower body the way the game does (aligned at particle 8), with an upper-body twist slider (in game, up to about 37° while running and 60° while walking; about 93° when moving while aiming, beyond which the game walks backwards with the legs turned toward the crosshair). Particles cannot be dragged during the preview, and the edited data is untouched. The upper layer is optional: falling, dive, climbing a ladder, swimming, going/leaving prone, surrender, skydiving and wounded are played as one whole-body animation in game, and picking one of those says so.
...
Sprite size is measured in **pixels** by default, as in game: the game hardcodes the edge length per detail level (high detail outline 4.2 / body 2.6, low detail 3.6 / 2.4) and leaves distance attenuation off, so voxels do not grow when the camera moves closer. That also means it only matches the real thing at the game's camera distance and a similar window height: the "Game camera" button next to it sets perspective, 1152 voxels away and 58.4° above the horizon. Turn "Size in pixels" off for the old behaviour that follows zoom.
