...
- **Symmetry optimizer** (binding tool): the game picks each stick's skinning rule by index, and only some index pairs are mirror images of each other. "Symmetry optimizer..." can load the model's own animations, search for a slot assignment that keeps both sides symmetric without making the voxels follow their sticks worse, list each pair's current and suggested cost, and apply it in one undoable step. The stick panel can also "Reverse direction" and "Swap index" by hand; voxel bindings move with the sticks.
